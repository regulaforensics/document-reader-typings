...
export class ListVerifiedFieldContainer extends aContainer implements iListVerifiedFieldContainer {
  ...
  ): (ListVerifiedFieldContainer | iListVerifiedFieldContainer)[] {
    try {
      ...
      return asPlain
        ? result.map(
            (container) => instanceToPlain(container, { exposeUnsetFields: false }) as iListVerifiedFieldContainer,
          )
        : result
    } catch (error) {
      return []
  ...
  static validate = (instance: ListVerifiedFieldContainer): true | never => {
    ...
    if (errors.length) {
      throw new DocReaderTypeError(
        'ListVerifiedFieldContainer validation error: the data received does not match model structure!',
        errors,
      )
    }
